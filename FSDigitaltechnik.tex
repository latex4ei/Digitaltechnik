...
\symbolbox{
\begin{tabular*}{\columnwidth}{@{\extracolsep}r l r l  r l @{}}
$Z$ & Zahl & $p$& \#Ziffern vorne  & $d_i$ & Ziffer \\
...
Benötigte Bits: $N: n$ Bit  $M: 2n$ Bit \quad
$N + M = 2n +1$ Bit \quad
$N \cdot M = 3n$ Bit
}
...
\tablebox{
	...
	\ctrule
		& Isolator & Metall & undotiert & N-Typ & P-Typ \\ \cmrule
...
\cookbox{Vorgehen}{
	\item Doppeltes Negieren der Funktion: $ Z = \overline {\overline{\overline A \cdot \overline B + \overline C \cdot D}}$ \\ 
	\item Umformung "'untere"'  Negation (DeMorgan) : $ Z = \ol{\ol{\ol A \cdot \ol B} \cdot \ol{\ol C \cdot D}} = \ol{(A+B) \cdot (C+\ol D)}$ \\ 
	\item Ausmultiplizieren: $ Z = \ol{(A+B) \cdot (C+\ol D)} = \ol{A\cdot C + A\cdot \ol D + B \cdot C + B \cdot \ol D}$ \\ 
	\item Umformung "'obere"'  Negation (DeMorgan) :\\ $ Z= \ol{AC} \cdot \ol{A \ol D} \cdot \ol{BC} \cdot \ol{B\ol D} = ( \ol A + \ol C) \cdot ( \ol A + D) \cdot ( \ol B + \ol C) \cdot ( \ol B + D)$ \\ 
	\item Analog von KNF nach DNF.
...
Drei Grundgatter der CMOS-Technologie:\\
	...
	\emph{Vorteil:}	 (Fast) nur bei Schaltvorgängen Verlustleistung - wenig statische Verluste
		...
		\tablebox{
			...
			\ctrule
				Netzwerk & Pull-Dow\bf{n} & Pull-U\bf{p} \\ \cmrule
	...
	\begin{tabular}{ll}
		\emph{Dynamische Verlustleistung}	& $P_{\ir dyn} = P_{\ir cap} + P_{\ir short}$\\
		\quad Kapazitive Verluste & $P_{\ir cap} = \alpha_{01} f C_L V_{DD}^2$\\
		\quad Kurzschlussstrom	& $P_{\ir short} = \alpha_{01} f \beta_n \tau (V_{DD} - 2V_{tn})^3$\\[0.8em]
	...
	\emph{Statische Verlustleistung} $P_{\ir stat}$: Sub-Schwellströme, Leckströme, Gate-Ströme
	
	Abhängigkeit: $V_{DD}\uparrow:P_{\ir stat}\uparrow$ \qquad $V_{\ir th}\uparrow:P_{\ir stat}\downarrow$ \quad (aber nicht proportional)\\
...
\tablebox{
	...
	\ctrule
		$t_{\ir Setup}$ & Stabilitätszeit vor der aktiven Taktflanke\\
$t_{\ir hold}$ & Stabilitätszeit nach  der aktiven Taktflanke\\
$t_{\ir c2q}$ & Eingang wird spätestens nach $t_{\ir c2q}$ am Ausgang verfügbar\\
Max. Taktperiode &  $t_{\ir clk} \ge t_{1,c2q} + t_{\ir logic,max} + t_{\ir 2,setup}$  \\
 Max. Taktfrequenz & $f_{\ir max} = \left\lfloor \frac{1}{t_{\ir clk}} \right\rfloor$ \qquad (Nicht aufrunden) \\
 Holdzeitbedingung & $t_{\ir hold} \le t_{\ir c2q} + t_{\ir logic,min}$  $\ra$ Dummy Gatter einbauen\\
 Durchsatz & $\frac{1 \text{Sample}}{t_{\ir clk,pipe}} = f$ \\
 Latenz & $t_{\ir clk} \cdot \#$Pipelinestufen (das zwischen den FFs) \\ 
	\ctrule
	\end{tabular*}
}
...
Durchsatz = $\frac{\#\text{Modul}}{t_{\ir clk,Modul}} = f$ \qquad \quad Latenz = $t_{\ir clk}$
\begin{itemize} \itemsep0pt
...
\sectionbox{
	...
	\begin{description}
	\item[Flüchtig] Speicherinhalt gehen verloren, wenn Versorgungsspannung $V_{DD}$ wegfällt - Bsp: *RAM
	\item[Nicht Flüchtig] Speicherinhalt bleibt auch ohne $V_{DD}$ erhalten - Bsp: Flash
	\item[Asynchron] Daten werden sofort geschrieben/gelesen.
	\item[Synchron] Daten werden erst mit $clk_{0 \ra 1}$ geschrieben.
	\item[Dynamisch] Ohne Refreshzyklen gehen auch bei angelegter $V_{DD}$ Daten verloren -  Bsp: DRAM
	\item[Statisch] Behält den Zustand bei solange $V_{DD}$ anliegt (keine Refreshzyklen nötig) - Bsp: SRAM
