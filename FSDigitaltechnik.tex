...
$N \cdot M: n+m$ Bit
	...
	\subsection{Zweierkomplement \qquad Wertebereich: $-2^{n-1} \le Z \le 2^{n-1} -1$}
		\begin{tabular}{l|l}
			\textbf{$Z \rightarrow -Z$} & Bsp: Wandle 2 in -2 um\\
	...
	\begin{tabular}{l|l}
		...
		$Z = (-1)^s \cdot  1.f \cdot 2^{e-127}$ & $Z=-1 \cdot 2^{-1} \cdot 1.01_2 = -0.101_2= -0.625$\\ \\ 
		\textbf{Wert $Z$ $\rightarrow$ IEEE} & Bsp: $Z=11.25$ \\
		$s=0$(positiv), $s=1$(negativ) & $s=0$\\
		$Z_{10} \rightarrow Z_2$ (beim Komma teilen) & $Z = 1011.01_2$\\
		$Z_2$ n-mal shiften $\rightarrow$ $1.xxx\dots$ & $Z=1.01101_2\cdot 2^3$ \\
		Exponent $e_{10}=n+127 \rightarrow e_2$ & $e=3+127=130=10000010_2$\\
		Mantisse $f_2=xxx\dots$ & $f=01101000\dots_2$
	\end{tabular}
		\\[0.5em]
		\textbf{Spezialwerte:} $Wert = 0 \Leftrightarrow e=0$ \qquad $Wert = \infty \Leftrightarrow e=255$
...
\section{Beschreibungsformen}
	...
	\textbf{Eins-Zeilen} als \textbf{Minterme} schreiben und alle Minterme mit \textbf{ODER} verknüpfen: \\
	$Z = \overline A \cdot \overline B + \overline C \cdot D$
	
	\subsection{Konjunktive Normalform/Product of sums (KNF/POS)}
	\textbf{Null-Zeilen negiert als Maxterm} schreiben und \textbf{UND} verknüpfen: \\	
...
\parbox{4.0cm}{ \includegraphics{./img/ds/mosfet.pdf} \\ $V_{Pinch-Off} = V_{GS} - V_{th}$ } \parbox{3.0cm}{ \includegraphics{./img/ds/char_nmos.pdf} } 
	...
	Verstärkung: \framebox{
			$\beta = K' \frac{W}{L} \text{ mit } K' = \frac{\mu \varepsilon_{ox} \varepsilon_0}{t_{0x}} $
		} \\
	\begin{tabular} {r | l}
		Kanalweite & W  \\
		Kanallänge & L  \\
...
Drei Grundgatter der CMOS-Technologie:\\
	...
	\begin{tabular}{l|l|l}
		Netzwerk & Pull-Dow\bf{n} & Pull-U\bf{p} \\ \hline
...
\section{Logikminimierung}
	...
	\begin{itemize}\itemsep0pt
	\item $m_i$ Minterm: UND-Term in dem alle Variablen vorkommen
	\item $M_i$ Maxterm: ODER-Term in dem alle Variablen vorkommen
